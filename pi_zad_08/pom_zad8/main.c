...
void *bsearch2(const void *key, const void *base, size_t nmemb, size_t size, ComparFp compar, char *result){
    ...
    while(left<=right){
        ...
        else if(compar(base+mid*size,key)==1){
            right=base+mid*size-1;
        }
        else if(compar(base+mid*size,key)==1){
            left=base+mid*size+1;
        }
    }
    *result=0;
    return base+left*size;
}
...
int main(void) {
	...
	switch (to_do) {
		...
		default:{
		    Date data1={1,2,2002};
		    Date data2={1,4,2002};
		    Food burger1={"burger", 4.50, 3, data1};
		    Food burger2={"burger2", 3.50,2, data2};
		    Food *b1=&burger1;
		    Food *b2=&burger2;
		    printf("%d",cmp(&b1,&b2));
