...
/*
int cmp_names(char str1[], char str2[], float pr1, float pr2){
...
int cmp_names(float pr1, float pr2, int day1, int day2, int month1, int month2, int year1, int year2){
    if(pr1>pr2)
        return 1;
    else if(pr1<pr2)
        return 2;
...
int bsearch2 (Food key, Food base[], int n, int size, int *res) {
    ...
    while(left<=right){
        ...
        temp=cmp_names(key.price, base[mid].price, key.valid_date.day, base[mid].valid_date.day, key.valid_date.month, base[mid].valid_date.month, key.valid_date.year, base[mid].valid_date.year);
        if(temp==0){
            *res=1;
            return mid;
        }
